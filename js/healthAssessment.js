...
function bindQuestion () {
    ajaxCrossDomainGet(config_serviceEndPoint + 'GetQData?QTID=' + sessionStorage.getItem('HEALTH_ASSESSMENT_QTID') + '&src=00&descOrder=true' , bindQuestion_callback, 'post error');
}
...
function bindQuestion_callback(msg) {
    ...
    if (qtid == '00050'){
        for (var i = 0; i < data.length; i++){
            ...
            if ( data[i]['QID'] == '00001') {
                ...
                $form.append('<input id="'+ id  +'" type="number" class="form-control" pattern="[0-9]*" placeholder="ส่วนสูง (เซนติเมตร)">');
                $form.find('input[type=number]').keydown(numericValidation);
            } else if ( data[i]['QID'] == '00002') {
                $form.append('<label>' + data[i]['text'] + '</label>');
                var id = data[i]['QTID'] + ':' + data[i]['QID'] + ':' + data[i]['answer'][0]['id']; 
                $form.append('<input id="'+ id  +'" type="number" class="form-control" pattern="[0-9]*" placeholder="น้ำหนัก (กิโลกรัม)">');
                $form.find('input[type=number]').keydown(numericValidation);
            } else if ( data[i]['QID'] == '00003') {
                $form.append('<label>' + data[i]['text'] + '</label>');
                var id = data[i]['QTID'] + ':' + data[i]['QID'] + ':' + data[i]['answer'][0]['id']; 
                $form.append('<input id="'+ id  +'" type="number" class="form-control" pattern="[0-9]*" placeholder="น้ำตาลในเลือด (มิลลิกรัม)">');
                $form.find('input[type=number]').keydown(numericValidation);
            } else if ( data[i]['QID'] == '00004') {
                $form.append('<label>' + data[i]['text'] + '</label>');
                var id = data[i]['QTID'] + ':' + data[i]['QID'] + ':' + data[i]['answer'][0]['id']; 
                $form.append('<input id="'+ id  +'" type="text" class="form-control" placeholder="ระดับความดันโลหิต (มิลลิเมตรปรอท)">');
                $form.find('input[type=number]').keydown(numericValidation);
            } else {
                $form.append('<label>' + data[i]['text'] + '</label>');
    ...
    if ( sessionStorage.getItem('HEALTH_ASSESSMENT_EDIT') ) { loadUserData(); }
}
...
function loadUserData () {
    ...
    ajaxCrossDomainGet(get_url, loadUserData_callback, 'getUserData er');
}
